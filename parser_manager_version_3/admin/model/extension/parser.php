...
class ModelExtensionParser extends Model
{
    ...
    public function SearchProducts($data = array())
    {
        ...
        if (!empty($data['filter_category'])) {
            foreach ($data['filter_category'] as $filter_category){
                $sql .= " AND p2c.category_id = " . $this->db->escape($filter_category);
            }
        ...
        if (!empty($data['filter_model'])) {
            $sql .= " AND LCASE(p.mol) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_model'])) . "%'";
        }
    ...
    public function updateParserUrls()
    {
        ...
        if (empty($result)) {
            $result = 'Обновление не требуется!';
        }
        return $result;
    }
