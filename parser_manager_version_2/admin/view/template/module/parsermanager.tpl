...
<div id="setting">
<form id="form_setting" action="<?php echo $base."index.php?route=module/parsermanager/index&token=".$token;?>" method="POST" >
<fieldset>
...
<a href="<?php echo $base."index.php?route=module/parsermanager/parselog&token=".$token;?>"  target="_blank" style="padding-left:10px;padding-top:0px;float:right;">Показать лог</a>
<a href="<?php echo $base."index.php?route=module/parsermanager/viewPage&token=".$token;?>"  target="_blank" style="padding-left:10px;padding-top:0px;float:right;">Показать страницу</a>
...
<tr style="border-top: 1px solid #DDDDDD;">
	...
	<td style="padding-left:5px;">
		<a href="<?php echo HTTP_SERVER."index.php?route=module/parsermanager/updateTableParser&token=".$token;?>">Обновить таблицы парсера</a>								
	</td>
</tr>
<tr style="border-top: 1px solid #DDDDDD;">
            ...
            <tr >
                <td >
                    <div class="alert alert-danger"style="text-align: right ;">
                        ...
                        <a class="btn btn-primary btn-xs" onclick="parsingAllProducts();" class="button"><?php echo 'Парсить дальше' ?></a>
                        <a class="btn btn-primary btn-xs" onclick="clearTableUrls();" class="button"><?php echo 'Очистить список' ?></a>
                    </div>
                </td>
            </tr>
        </table>
            ...
            <fieldset><legend>Обновление существующих товаров</legend>
                ...
                <table id="prod_table" class="table table-bordered table-hover">
                    ...
                    <tr class="left1" >
                        <td></td>
                        <td></td>
                        <td><input type="text" id="filter_name"  value="<?php if (isset($setting['filter_name'])) echo $setting['filter_name'];?>" size = "27" name="filter_name" class="ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true" value="<?php  echo $setting['filter_name'];?>"></td>
                    ...
                    <td  class="left2" name='<?php echo $product['product_id'];?>' style="width:280px;">					   
                        <?php if (!empty($product['url_parsing'])) {?>
							<div  class="delete_url" >							
								...
								<div  style="float:right;">
									<a  class="btn btn-danger" title="Удалить ссылку" onclick="deleteUrl(<?php echo $product['product_id'];?>);"><i class='fa fa-trash-o'/> </i></a>
								</div>
							</div>
                        <?php } else {?>
						<div class="add-url-product" style="float:right;">
...
$(document).ready(function () {
   ...
   $(document).delegate('a[id=image-path]', 'click', function() {        
        ...
        $.ajax({
            url: 'index.php?route=module/parsermanager/foldermanager&token=<?php echo $token; ?>',
            dataType: 'html',
            beforeSend: function() {
    ...
    $('#filter_name').autocomplete({
        ...
        source: function(request, response) {
            $.ajax({
                url: 'index.php?route=module/parsermanager/autocompleteparserj&token=<?php echo $token; ?>&filter_name=' +  $('#filter_name').val(),
                dataType: 'json',
  	...
  	$('input[name=\'filter_model\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=module/parsermanager/autocompleteparserj&token=<?php echo $token; ?>&filter_model=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
...
function deleteUrl(product_id) {
    ...
    $.ajax({
        url: "index.php?route=module/parsermanager/deleteUrl&token=<?php echo $token; ?>&product_id="+product_id,
        dataType: "html",
...
function proxy_check_ajax() {
    $.ajax({
        url: "index.php?route=module/parsermanager/proxycheck&token=<?php echo $token; ?>",
        dataType: "html",
		beforeSend: function() {
...
function parsingAllProducts() {
    ...
    $('#form_parsing').attr('action', '<?php echo $base."index.php?route=module/parsermanager/parsingAllproducts&token=".$token;?>'+'&product_id='+filter_value+page);
    $('#form_parsing').submit();
...
function searchAllProducts() {
    ...
    $('#form_parsing').attr('action', '<?php echo $base."index.php?route=module/parsermanager/searchAllproducts&token=".$token;?>'+'&product_id='+filter_value+page);
    $('#form_parsing').submit();
...
function parsingProduct(product_id) {
    ...
    $('#form_parsing').attr('action', '<?php echo $base."index.php?route=module/parsermanager/parseProduct&token=";?>'+getURLVar('token')+'&product_id='+product_id+'&price='+price+filter_value+page);
    $('#form_parsing').submit();

}

function filter() {
    var url = "index.php?route=module/parsermanager&token=<?php echo $token; ?>";
...
function deleteProduct() {
    ...
    var url = "index.php?route=module/parsermanager/delete&token=<?php echo $token; ?>";
...
function addProduct() {
    var url = "index.php?route=module/parsermanager/addproductparser&token=<?php echo $token; ?>";
    var filter_name = $('#filter_name').attr('value');
    var filter_sku_exist = $('#filter_sku_exist').attr('value');
    var filter_model = $('#filter_model').attr('value');
...
function find_product(product_id, product_name) {
    $('#modal-image').remove();
    $.ajax({
        url: 'index.php?route=module/parsermanager/findmanager&token=<?php echo $token; ?>&product_id=' + product_id + '&product_name=' + product_name ,
        dataType: 'html',
        beforeSend: function() {
...
function addUrlAjax(product_id, url_parsing){   
    $.ajax({
        ...
        url: "index.php?route=module/parsermanager/addurl&token=<?php echo $token; ?>",
        dataType: "html",
        success: function(msg){			
...
function clearTableUrls(){
    $.ajax({
        url: "index.php?route=module/parsermanager/clearTableUrls&token=<?php echo $token; ?>",
        dataType: "html",
        success: function(msg){
            $('#error-table').hide();
            alert(msg);
...
function delete_cookie(){
    $.ajax({
        url: "index.php?route=module/parsermanager/delete_cookie&token=<?php echo $token; ?>",
        dataType: "html",
        success: function(msg){
            alert(msg);
        },
...
{ 
	$.ajaxFileUpload
		({
			url:'index.php?route=module/parsermanager/ajaxUploadFile&token=<?php echo $token; ?>',
			secureuri:false,
